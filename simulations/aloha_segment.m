...
for (iter = 1:num_max_times)   
    ...
    while (~done)
        ...
        % Adjust N dynamically for next round.
        if ((s0 == 0) & (s1 == 0)) % If only collision slots.
            N = 2 * N;
        else
        	% Find max and min timestamps of tags that got through (single occupancy time slots).
            T = 0;
            if (s1 > 0)
                ...
                max_time = max(later_times(single_tag_indices));
                min_time = min(later_times(single_tag_indices));
                T = max_time - min_time;
                max_time_changed = true;
            ...
            n_est = ceil(n_est - ((1-p)/p)*lambda*T); % Adjust for the tags we are going to ignore in the next iteration.
            if (n_est < 0) n_est = 1; end
